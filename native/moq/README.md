...
```elixir
{:ok, session} = setup_session(url, self(), false)
...
{:ok, broadcast} = open_broadcast(session, "my_broadcast")

{:ok, audio_track} = add_aac_track(broadcast, "my_audio_track", profile, sample_rate, channels)
{:ok, video_track} = add_h264_track(broadcast, "my_video_track", params, dcr, codec)

Stream.repeatedly(fn ->
  receive do
    {:audio, buf, timestamp_us} -> send_frame(audio_track, timestamp_us, true, buf)
    {:video, buf, timestamp_us, keyframe?} -> send_frame(video_track, timestamp_us, keyframe?, buf)
  end
end)
```
